...
public class Velocity extends Component implements Summable {
	...
	public Velocity(Vector2f vector, int mass, double maxVelocity,
			double maxForce, double panicVelocity) {
		this.vector = vector;
		this.mass = mass;
	...
	public static Velocity RandomVelocity(int mass, double maxVelocity,
			double maxForce, double panicVelocity) {
		Random r = new Random();
		float x = (r.nextFloat() * 2) - 1;
		float y = (r.nextFloat() * 2) - 1;
		Vector2f randomVector = new Vector2f(x, y).normalise();
		Velocity randomVelocity = new Velocity(randomVector, mass, maxVelocity,
				maxForce, panicVelocity);
